package com.retail.dolphinpos.data.work_manager
...
class PosSyncWorker @AssistedInject constructor(
    ...
    private suspend fun executeCloseBatch(command: SyncCommandEntity) {
        ...
        // Verify all orders for this batch are synced (optional, rely on FIFO)
        // For now, we assume FIFO ensures orders are synced before batch close.
        // A more robust check would involve querying for unsynced orders for this batch.

        // Create API request
        val request = BatchCloseRequest(
            cashierId = batch.userId ?: throw IllegalArgumentException("userId is null"),
            closedBy = batch.userId, // Assuming closedBy is the same as cashierId
            closingCashAmount = batch.closingCashAmount ?: 0.0, // Default to 0 if null
            locationId = batch.locationId ?: throw IllegalArgumentException("locationId is null"),
            orders = emptyList(), // Abandon orders are not part of this sync flow
            paxBatchNo = "", // PAX batch handling is separate
            storeId = batch.storeId ?: throw IllegalArgumentException("storeId is null")
        )

        // Call API (idempotent - backend should handle if already closed)
        apiService.batchClose(batchId, request)
