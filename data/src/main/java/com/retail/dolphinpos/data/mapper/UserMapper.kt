...
import com.retail.dolphinpos.domain.model.auth.batch.BatchSyncStatus as DomainBatchSyncStatus
import com.retail.dolphinpos.domain.model.auth.batch.BatchWithSyncStatus
import com.retail.dolphinpos.data.entities.user.BatchSyncStatus as DataBatchSyncStatus
...
object UserMapper {
    ...
    fun toBatchDetails(batchEntity: BatchEntity): Batch {
        return Batch(
            // Note: Old Batch model uses Int batchId, but POS always uses UUID String
            // For backward compatibility, we use 0 since batchId is always UUID in new system
            batchId = 0,
    ...
    // -------------------------
    // BatchWithSyncStatus Mappers
    // -------------------------
    
    fun toBatchWithSyncStatus(batchEntity: com.retail.dolphinpos.data.entities.user.BatchEntity): BatchWithSyncStatus {
        return BatchWithSyncStatus(
            batchId = batchEntity.batchId, // Batch IDs are always generated by POS (UUID)
            batchNo = batchEntity.batchNo,
            userId = batchEntity.userId,
            storeId = batchEntity.storeId,
            registerId = batchEntity.registerId,
            locationId = batchEntity.locationId,
            startingCashAmount = batchEntity.startingCashAmount,
            startedAt = batchEntity.openedAt, // Use openedAt instead of startedAt
            closedAt = batchEntity.closedAt,
            closingCashAmount = batchEntity.closingCashAmount,
            syncStatus = toDomainBatchSyncStatus(batchEntity.syncStatus)
        )
    }
    
    fun toBatchEntity(batch: BatchWithSyncStatus): com.retail.dolphinpos.data.entities.user.BatchEntity {
        return com.retail.dolphinpos.data.entities.user.BatchEntity(
            batchId = batch.batchId, // Batch IDs are always generated by POS (UUID)
            batchNo = batch.batchNo,
            userId = batch.userId,
            storeId = batch.storeId,
            registerId = batch.registerId,
            locationId = batch.locationId,
            startingCashAmount = batch.startingCashAmount,
            openedAt = batch.startedAt, // Use startedAt from domain as openedAt
            closedAt = batch.closedAt,
            closingCashAmount = batch.closingCashAmount,
            lifecycleStatus = if (batch.closedAt == null) 
                com.retail.dolphinpos.data.entities.user.BatchLifecycleStatus.OPEN 
            else 
                com.retail.dolphinpos.data.entities.user.BatchLifecycleStatus.CLOSED,
            syncStatus = toDataBatchSyncStatus(batch.syncStatus),
            startedAt = batch.startedAt, // Keep for backward compatibility
            isSynced = batch.syncStatus != DomainBatchSyncStatus.START_PENDING // Keep for backward compatibility
        )
    }
    
    private fun toDomainBatchSyncStatus(dataStatus: DataBatchSyncStatus): DomainBatchSyncStatus {
        return when (dataStatus) {
            DataBatchSyncStatus.START_PENDING -> DomainBatchSyncStatus.START_PENDING
            DataBatchSyncStatus.START_SYNCED -> DomainBatchSyncStatus.START_SYNCED
            DataBatchSyncStatus.CLOSE_PENDING -> DomainBatchSyncStatus.CLOSE_PENDING
            DataBatchSyncStatus.CLOSE_SYNCED -> DomainBatchSyncStatus.CLOSE_SYNCED
            DataBatchSyncStatus.FAILED -> DomainBatchSyncStatus.FAILED
        }
    }
    
    private fun toDataBatchSyncStatus(domainStatus: DomainBatchSyncStatus): DataBatchSyncStatus {
        return when (domainStatus) {
            DomainBatchSyncStatus.START_PENDING -> DataBatchSyncStatus.START_PENDING
            DomainBatchSyncStatus.START_SYNCED -> DataBatchSyncStatus.START_SYNCED
            DomainBatchSyncStatus.CLOSE_PENDING -> DataBatchSyncStatus.CLOSE_PENDING
            DomainBatchSyncStatus.CLOSE_SYNCED -> DataBatchSyncStatus.CLOSE_SYNCED
            DomainBatchSyncStatus.FAILED -> DataBatchSyncStatus.FAILED
        }
    }
